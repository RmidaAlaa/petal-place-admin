                          ...
                          {recentOrders.map((order) => (
                            <TableRow
                              ...
                              onClick={() => setSelectedOrder({
                                ...
                                items: [
                                  { id: '1', name: 'Red Rose Bouquet', quantity: 1, price: 89.99 },
                                  { id: '2', name: 'Greeting Card', quantity: 1, price: 5.00 }
                                ],
                                total: order.total,
                                status: order.status,
                                tracking: {
                                  status: order.status,
                                  location: order.status === 'shipped' ? 'Riyadh Distribution Center' : order.status === 'delivered' ? 'Customer Address' : 'Processing Center',
                                  estimatedDelivery: order.status === 'delivered' ? 'Delivered' : '2024-01-20',
                                  updates: [
                                    { timestamp: '2024-01-13T10:00:00Z', status: 'confirmed', description: 'Order confirmed' },
                                    { timestamp: '2024-01-14T14:30:00Z', status: 'processing', description: 'Order being prepared' },
                                    { timestamp: '2024-01-15T09:15:00Z', status: 'shipped', description: 'Order shipped from warehouse' },
                                    ...(order.status === 'delivered' ? [{ timestamp: '2024-01-16T16:45:00Z', status: 'delivered', description: 'Order delivered successfully' }] : [])
                                  ]
                                },
                                shippingAddress: '123 Main St, Riyadh, Saudi Arabia',
                                billingAddress: '123 Main St, Riyadh, Saudi Arabia',
                                paymentMethod: 'Credit Card',
                                orderDate: order.date
                              })}
                            >
                              <TableCell className="font-medium cursor-pointer">{order.id}</TableCell>
                      ...
                      <CardContent className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-medium">{selectedOrder.tracking.location}</span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-sm">Estimated Delivery</span>
                              <span className="text-sm font-medium">{selectedOrder.tracking.estimatedDelivery}</span>
                            </div>

                            {/* Delivery Timeline */}
                            <div className="mt-4">
                              <h5 className="text-sm font-medium mb-3">Delivery Timeline</h5>
                              <DeliveryTimeline
                                currentStatus={selectedOrder.status}
                                trackingEntries={selectedOrder.tracking.updates.map(update => ({
                                  id: update.timestamp,
                                  status: update.status,
                                  description: update.description,
                                  timestamp: update.timestamp
                                }))}
                                estimatedDelivery={selectedOrder.tracking.estimatedDelivery !== 'Delivered'
                                  ? new Date(selectedOrder.tracking.estimatedDelivery)
                                  : undefined}
                              />
                            </div>
                          </div>
                        ...
                        <div>
                          ...
                          <div className="text-sm space-y-1">
                            ...
                            <div>Order Date: {new Date(selectedOrder.orderDate).toLocaleDateString()}</div>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
